...
<h1 align="center">Gald3r for Cline</h1>

<p align="center">
  The <strong>Cline edition</strong> of Gald3r -- file-based memory, task management, and
  agent orchestration that runs <strong>inside Cline</strong>. Part of the Gald3r framework
...
This is the **Cline distribution** of Gald3r. The [`cline/`](./cline/) folder is a
complete, ready-to-deploy gald3r setup tuned for Cline -- drop its contents into your
...
```bash
git clone https://github.com/wrm3/gald3r_platform_cline.git
cp -r gald3r_platform_cline/cline/. /path/to/your/project/
```

Then open the project in Cline and type `@g-status` to confirm gald3r loaded.

> The `cline/` payload is **additive** -- it drops in gald3r's config + `.gald3r/` state and
